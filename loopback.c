...
struct decode *dec;
extern int quitflag;
/* extern struct capture_testbuffer cap_buffers[];   move to struct instance_priv */
...
encode(void)
{
	...
	ret = v4l_get_capture_data(&v4l2_buf, enc->cmdl->instns);
	if (ret < 0) {
		return -1;
	}
	...
	fb[src_fbid].bufY = ins_priv[enc->cmdl->instns].cap_buffers[v4l2_buf.index].offset;
	...
	frame_id++;
	v4l_put_capture_data(&v4l2_buf, enc->cmdl->instns);
...
encdec_test(void *arg)
{
	...
	/* start capture */
	ret = v4l_start_capturing(cmdl->instns);
	if (ret < 0) {
...
err5:
	v4l_stop_capturing(cmdl->instns);
err4:
	decoder_close(dec);
